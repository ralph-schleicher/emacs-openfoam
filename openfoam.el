...
and project directory."
  ...
  (let* ((working-directory (file-name-as-directory
	...
	(openfoam-shell-wait-for-prompt)
	(let ((last-output (point))
	      (path (shell-snarf-envar "PATH"))
	      (sep (regexp-quote path-separator)))
	  ;; Kill output generated by ‘shell-snarf-envar’.
	  (openfoam-shell-wait-for-prompt)
	  (comint-kill-region last-output (point))
	  ;; Remove all text properties and append ‘exec-directory’
	  ;; to the program search path.
	  (set-text-properties 0 (length path) () path)
